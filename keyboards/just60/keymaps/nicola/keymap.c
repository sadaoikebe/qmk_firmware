...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_QWERTY] = LAYOUT( \
      ...
      MO(_FUNC),   KC_LGUI, KC_LALT,      KC_NO,          KC_SPC,  KC_SPC,          KC_NO,   KC_RALT, KC_RGUI,  KC_APP,  KC_RCTL  ),
...
// NICOLA親指シフト
  // デフォルトレイヤーに関係なくQWERTYで
    [_NICOLA] = LAYOUT( \
      ...
      MO(_FUNC),   KC_LGUI, KC_LALT,      KC_NO,            NG_SHFTR, NG_SHFTL,       KC_NO,   KC_RALT, KC_RGUI,  KC_APP,  KC_RCTL  ),
  ...
  [_FUNC] = LAYOUT( \
      ...
      MO(_FUNC),   KC_NO,   KC_NO,        KC_NO,           KC_KANA2, KC_EISU,          KC_NO,    KC_NO,   KC_NO,   KC_NO,   KC_NO   )
  ...
  switch (keycode) {
    case KC_EISU:
      if (record->event.pressed) {
        // NICOLA親指シフト
        send_string(SS_RCTL(SS_RSFT(SS_TAP(X_F13))));
        nicola_off();
        // NICOLA親指シフト
        update_led();
      }
    ...
    case KC_KANA2:
      if (record->event.pressed) {
        // NICOLA親指シフト
        send_string(SS_RCTL(SS_RSFT(SS_TAP(X_F14))));
        nicola_on();
        // NICOLA親指シフト
        update_led();
      }
